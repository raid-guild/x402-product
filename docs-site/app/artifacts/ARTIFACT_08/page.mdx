...
- **Run a focused discovery campaign before scoping further engineering work**: conduct interviews and synthesize the findings to decide which positioning option to adopt, using the hypotheses and references in the [Customer Discovery](#customer-discovery) section.
- **Prototype the offer, not the product**: draft a set of white-label service packages with scope, pricing assumptions, and success metrics, drawing on operational costs and rate-limit constraints ([`artifact 3`](./ARTIFACT_03.md)), target segments and pricing ([`artifact 4`](./ARTIFACT_04.md)), and infrastructure/deployment assumptions ([`artifact 5`](./ARTIFACT_05.md), [`artifact 6`](./ARTIFACT_06.md)).
